...
ADMIN_VERB_ONLY_CONTEXT_MENU(show_player_panel, R_ADMIN, "Show Player Panel", mob/player in world)
	...
	log_admin("[key_name(user)] checked the individual player panel for [key_name(player)][isobserver(user.mob)?"":" while in game"].") // Splurt edit
	...
	if(player.client)
		...
		if(CONFIG_GET(flag/use_exp_tracking))
			...
			body += "<br>\[<a href='byond://?_src_=holder;[HrefToken()];getjobexemptwindow=[player.client.ckey]'>Job Exemption Menu</a>\]"
			// BUBBERS EDIT END
