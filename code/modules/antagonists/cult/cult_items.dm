...
/obj/item/shuttle_curse/attack_self(mob/living/user)
	...
	if(SSshuttle.emergency.mode == SHUTTLE_CALL)
		...
		if(totalcurses >= MAX_SHUTTLE_CURSES && (world.time < first_curse_time + SHUTTLE_CURSE_OMFG_TIMESPAN))
			...
			addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(priority_announce), omfg_message, "ВНИМАНИЕ: Приоритетная Тревога", 'sound/announcer/announcement/announce_syndi.ogg', null, "Департамент транспорта Nanotrasen"), rand(2 SECONDS, 6 SECONDS))
