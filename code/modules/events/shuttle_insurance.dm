...
/datum/round_event/shuttle_insurance/start()
	insurance_message = new("Страхование шаттла", "Привет, приятель, это [ship_name]. Не могу не заметить, что у тебя дикий и сумасшедший шаттл БЕЗ СТРАХОВКИ! Безумие. А что, если с ним что-нибудь случится?! Мы провели быструю оценку ваших тарифов в этом секторе и предлагаем [insurance_evaluation], чтобы покрыть ваш шаттл в случае любой катастрофы.", list("Приобрести страховку.","Отклонить предложение."))
...
/datum/round_event/shuttle_insurance/proc/answered()
	if(EMERGENCY_AT_LEAST_DOCKED)
		priority_announce("Друзья, вы определенно опоздали с покупкой страховки. Наши агенты не работают на месте.",sender_override = ship_name, color_override = "red")
		return
	if(insurance_message && insurance_message.answered == 1)
		var/datum/bank_account/station_balance = SSeconomy.get_dep_account(ACCOUNT_CAR)
		if(!station_balance?.adjust_money(-insurance_evaluation))
			priority_announce("Вы не прислали нам достаточно денег на страховку шаттла. На языке космических дилетантов это считается мошенничеством. Мы оставим ваши деньги себе, мошенники!", sender_override = ship_name, color_override = "red")
			return
