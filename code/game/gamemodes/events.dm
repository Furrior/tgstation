 ...
 */
/proc/power_failure()
	priority_announce("В электросети станции [station_name()] обнаружена аномальная активность. В качестве меры предосторожности электроснабжение станции будет отключено на неопределённый срок.", "ВНИМАНИЕ: Сбой Энергии", ANNOUNCER_POWEROFF)
 ...
 */
/proc/power_restore()
	priority_announce("Электроснабжение [station_name()] восстановлено. Приносим извинения за доставленные неудобства.", "Энергия восстановлена", ANNOUNCER_POWERON)
	for(var/obj/machinery/power/apc/C as anything in SSmachines.get_machines_by_type_and_subtypes(/obj/machinery/power/apc))
		if(C.cell && is_station_level(C.z))
