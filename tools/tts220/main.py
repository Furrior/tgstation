...
class VoiceRequest(BaseModel):
    ...
    sfx: list[list[str]] = None


class FFmpegFlters(Enum):
    test = ["-filter:a", "highpass=f=1000"]
    ...
    radio_robot = ["-filter:a",
                   "afftfilt=real='hypot(re,im)*sin(0)':imag='hypot(re,im)*cos(0)':win_size=1024:overlap=0.5, deesser=i=0.4, volume=volume=1.5, highpass=f=1000, lowpass=f=3000, acrusher=1:1:50:0:log"]
...
filters_bindings = {
    "radio": FFmpegFlters.radio,
    "robot": FFmpegFlters.robot,
    "radio_robot": FFmpegFlters.radio_robot,
    "megaphone": FFmpegFlters.megaphone,
    "megaphone_robot": FFmpegFlters.megaphone_robot,
    "tg_robot": FFmpegFlters.tg_robot
}
...
async def apply_ffmpeg_filter(audio: bytes, ffmpeg_filter: FFmpegFlters) -> tuple[bytes, bytes]:
    ...
    print(f"ffmpeg result size: {len(stdout)} stderr = \n{ffmpeg_metadata_output}")
...
@app.post("/voice")
async def translate_voice(request: VoiceRequest) -> dict[str, bytes]:
    body = request.model_dump()
    response = await do_silero_request(body)
    r = ujson.loads(response)
    encoded_audio = r['results'][0]['audio']
    audio = base64.b64decode(encoded_audio)
    resulting_audios = {"pure": encoded_audio}
    for sfx_set in request.sfx:
        stdout = audio
        for sfx in sfx_set:
            if not filters_bindings.get(sfx):
                continue
            stdout, stderr = await apply_ffmpeg_filter(stdout, filters_bindings[sfx])
        encoded_result = base64.b64encode(stdout)
        resulting_audios["_".join(sfx_set)] = encoded_result
